...
macro( _HDF5_invoke_compiler language output return_value )
    if( HDF5_${language}_COMPILER_EXECUTABLE )
        exec_program( ${HDF5_${language}_COMPILER_EXECUTABLE} 
            ARGS -show
            OUTPUT_QUIET OUTPUT_VARIABLE ${output}
...
# If we don't have an HDF5_LIBRARY_DIRS on WIN32, look for hdf5dll in ${WITH_HDF}/dll.
# This finds the .lib version but we'll assume that the .dll exists in there, too.
if(WIN32 AND NOT HDF5_LIBRARY_DIRS)
    find_library(HDF5_DLL_LIBRARY NAMES hdf5dll
                 HINTS ${HDF5_ROOT}/dll)
    if(HDF5_DLL_LIBRARY)
        get_filename_component(HDF5_LIBRARY_DIRS ${HDF5_DLL_LIBRARY} PATH)
    endif()
endif()

# Check version number

# Use hdf5diff since that's found on WIN32
# Need to run this from the dll directory on WIN32 so the libraries are found.
if(WIN32)
    set(HDF5_WORKING_DIRECTORY ${HDF5_LIBRARY_DIRS})
else()
...
if(HDF5_VERSION)
    ...
    string(REPLACE "h5diff: Version " "" HDF5_VERSION ${HDF5_VERSION})
endif()

set(HDF5_VERSION_VALID)
if(HDF5_FIND_VERSION AND HDF5_FIND_VERSION_EXACT)
    if(HDF5_VERSION VERSION_EQUAL HDF5_FIND_VERSION)
        set(HDF5_VERSION_VALID 1)
    endif()
elseif(HDF5_FIND_VERSION)
    if(HDF5_VERSION VERSION_EQUAL HDF5_FIND_VERSION OR HDF5_VERSION VERSION_GREATER HDF5_FIND_VERSION)
        set(HDF5_VERSION_VALID 1)
    endif()
endif()

set(HDF5_ERROR_MESSAGE)
if(VSWIN64)
    set(HDF5_ERROR_MESSAGE "Could not find HDF5. Set the WITH_HDF environment variable.")
else()
